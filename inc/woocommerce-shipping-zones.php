<?php
/*
For Plugin: Bolder Fees for WooCommerce
	...
	if ( class_exists( 'WooCommerce' ) ) {
		...
		function create_new_tab() {
		    ...
		    if( WOOCOMMERCE_VERSION >= 2.1 ) 
				echo '<a href="' . admin_url( 'admin.php?page=wc-settings&tab=fee-regions' ) . '" class="nav-tab ';
			else
				echo '<a href="' . admin_url( 'admin.php?page=woocommerce_settings&tab=fee-regions' ) . '" class="nav-tab ';
			if( $current_tab == 'fee-regions' ) echo 'nav-tab-active';
			echo '">Fee Regions</a>';
		}
		...
		add_action('woocommerce_settings_tabs_fee-regions','be_table_rate_shipping_zones');
			...
			if(isset($zones[$zone_id]) && count($zones[$zone_id]) > 0) :
				...
				switch ($zone['zone_type']) {
		            ...
		            case 'countries':
		    			...
		    			if(in_array($country, $countries_abbr) || in_array($country.":".$state, $countries_abbr)) {
		    				if( ! empty( $zone['zone_except']['states'] ) && is_array( $zone['zone_except']['states'] ) && count( $zone['zone_except']['states'] ) ) {
		    				...
		    				if( ! empty( $zone['zone_except']['postals'] ) && is_array( $zone['zone_except']['postals'] ) && $zone['zone_except']['postals'] != '' ) {
